...
//   FirebaseFirestore.instance
//       .collection('LocationMark') // Ensure this matches the collection name
//       .snapshots()
//       .listen((snapshot) {
//     print("Firestore snapshot received: ${snapshot.docs.length} documents");

//     if (snapshot.docs.isEmpty) {
//       print("No documents in Firestore collection.");
//     }
...
//     mapMarkers.addAll(snapshot.docs.map((doc) {
//       return MapModel.fromMap(doc.id, doc.data());
//     }).toList());

//     print("Total markers after fetching: ${mapMarkers.length}");
//     mapMarkers.refresh(); // Force Obx to detect changes
//   });
// }
void fetchMarkers() async {
 ...
 Widget buildMap() {
  ...
  if (selectedMapType.value == 0) {
    ...
    filteredMarkers = mapMarkers.where((marker) {
      print("Checking Marker User ID: ${marker.userId}, Title: ${marker.title}");
      return marker.userId == userId;
    }).toList();

    print("Individual View - Markers for User ID $userId: ${filteredMarkers.length}");
  }
    ...
    child: Column(
      children: [
        ...
        ...filteredMarkers.map((marker) => ListTile(
              title: Text('${marker.title} at ${marker.latitude}, ${marker.longitude}'),
